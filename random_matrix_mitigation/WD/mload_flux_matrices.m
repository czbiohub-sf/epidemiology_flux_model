...
if exist('flux_matrix353.mat','file')
    disp('loading daily flux data for 353 days');
    tic;
    load('flux_matrix353.mat');
    toc;
    [N,nsamp]=size(M);
    if size(F,1)~=N || size(F,2)~=N || size(F,3)~=nsamp
...
toc;
clear('ff','q','s');
save('flux_matrix353.mat','F','M','-v7.3');